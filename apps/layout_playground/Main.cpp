...
public:
    PlaygroundComponent()
    {
        ...
        // Create and add the re-trigger overlay after other children so it paints on top
        reTriggerOverlay.reset (new ReTriggerOverlay (this));
        addAndMakeVisible (reTriggerOverlay.get());
        reTriggerOverlay->setBounds (getLocalBounds());
        // Force z-order: send ring to back and overlay to front so overlay is always visible above ring and other children
        if (ring) ring->toBack();
        reTriggerOverlay->toFront (true);
        // Also schedule an async enforcement of z-order after construction/layout to avoid races
        juce::MessageManager::callAsync ([this](){ if (ring) ring->toBack(); if (reTriggerOverlay) reTriggerOverlay->toFront (true); });
    }

    ~PlaygroundComponent() override
    {
        if (ring) removeChildComponent (ring.get());
        if (reTriggerOverlay) removeChildComponent (reTriggerOverlay.get());
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        // Fill the outer accent circle, then "cut out" the inner area by painting the base color
        // over the inner radius. This keeps the visual donut while avoiding dependence on Path
        // even-odd fill or unavailable clip APIs (we can switch to reduceClipRegion later).
        if (circles.size() > 0)
        {
            const auto& c0 = circles.getReference (0);
            const float maskOuter = 70.0f; // outer radius (matches ring->outerR)
            const float maskInner = 55.0f; // inner cutout radius (matches ring->innerR)

            // Create a Path consisting of the outer ellipse with an inner ellipse added.
            // Use even-odd fill so the inner ellipse becomes a hole and the ring child
            // (which paints after the parent) will be visible through it.
            juce::Path donutPath;
        ...
        // Draw shuffle selector (only the selected item is visible at a time)
        if (shufflePositions.size() > 0 && selectedShuffle >= 1 && selectedShuffle <= shufflePositions.size())
        {
        ...
        juce::String label;
        if (ringHoverSegment >= 0) label = "Offset start & trigger to step " + juce::String (ringHoverSegment + 1);
    ...
    void mouseDown (const juce::MouseEvent& e) override
    {
        ...
        if (idx >= 0)
        {
            // index 1 (re-trigger) is handled by the overlay child so the parent should not process it here
            if (idx != 1)
            {
    ...
    void mouseDrag (const juce::MouseEvent& e) override
    {
        ...
        {
            // pick nearest shuffle position to the drag point
    ...
    // Overlay component that draws the re-trigger inset circle and handles its mouseDown
    class ReTriggerOverlay  : public juce::Component
    {
    public:
        ReTriggerOverlay (PlaygroundComponent* ownerIn) : owner (ownerIn)
        {
        ...
        void paint (juce::Graphics& g) override
        {
            // Overlay no longer paints visuals; visuals are drawn by parent in paintOverChildren.
            juce::ignoreUnused (g, owner);
        }

        void mouseDown (const juce::MouseEvent& /*e*/) override
        {
            if (! owner) return;
            if (owner->ring) owner->ring->requestRestartAtNextMasterBoundary();
            owner->flashes.set (1, 1.0f);
            owner->startFadeTimer();
            owner->repaint();
        }
        ...
        {
            if (! owner) return;
            if (owner->hoverIndex == 1) owner->hoverIndex = -1;
            owner->repaint();
        }
        ...
        bool hitTest (int x, int y) override
        {
            ...
            if (owner->circles.size() <= 1) return false;
            const auto& c1 = owner->circles.getReference (1);
            const float insetR = 22.0f;
            const float f1 = juce::jlimit (0.0f, 1.0f, owner->flashes[1]);
            const float insetScale = 1.0f + 0.2f * f1;
            const float r = insetR * insetScale;
            const float dx = (float)x - c1.x;
            const float dy = (float)y - c1.y;
            return (dx*dx + dy*dy) <= (r * r);
        }

    private:
        PlaygroundComponent* owner = nullptr;
    };
    ...
    void resized() override
    {
        ...
        {
            reTriggerOverlay->setBounds (getLocalBounds());
            // keep overlay on top after any layout changes
            if (ring) ring->toBack();
            reTriggerOverlay->toFront (true);
        }
    }
...
private:
    ...
    std::unique_ptr<ReTriggerOverlay> reTriggerOverlay;
