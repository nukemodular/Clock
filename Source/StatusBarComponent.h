...
// Lightweight component encapsulating status text + LED pulse rendering.
// Parent sets status string and led level; component repaints only its own bounds.
...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        // Status text area (leave LED space on right ~20px)
        ...
        // LED circle on far right
        const float ledR = 6.0f; // matches UiLayout::kLedRadius
        auto cx = (float) (b.getRight() - 10);
        auto cy = (float) b.getCentreY();
        // Background shadow
        g.setColour(juce::Colours::black.withAlpha(0.5f));
        g.fillEllipse(cx - ledR - 1.5f, cy - ledR + 1.5f, ledR * 2.0f, ledR * 2.0f);
        // LED fill interpolated accent -> cyan brightness
